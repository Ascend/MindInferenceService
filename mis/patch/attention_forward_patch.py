    ...
    def forward(
        ...
        if hasattr(layer, 'quant_method') and False: # Attention weight quantization is not supported yet.
            ...
            self.seq_lens_tensor_cpu = torch.from_numpy(
                np.array(attn_metadata.prefill_metadata.seq_lens).astype(np.int32)) if isPrefill \
            ...
            if attn_metadata.num_prefills > 0:
                ...
                if (attn_metadata.block_tables is None
                    ...
                    self.seq_lens_tensor_cpu = torch.from_numpy(
                        np.array(attn_metadata.prefill_metadata.seq_lens).astype(np.int32))
                ...
                self.seq_lens_tensor_cpu = torch.from_numpy(
                    np.array(attn_metadata.decode_metadata.seq_lens).astype(np.int32))
